...
for xd in range(10000):
	...
	f='Build/NMF26F) Mozilla/5.0 (Linux; Android 8.1.0; S45B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.45 Mobile Safari/537.36'
	...
	ugen.append(uaku2)
for x in range(10):
	a='Mozilla/5.0 (Linux; Android 8.1.0; S45B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.45 Mobile Safari/537.36'
...
def banner():
	clear()
	print("""	
         \x1b[1;97m____ _  _ _    _   _    ___  ____ ____ _  _ 
         \x1b[1;97m|  | |\ | |     \_/     |__] |__/ |___ |\/
         \x1b[1;97m|__| | \| |___   |      |    |  \ |___ |  | 
                     \n\033[32mKEY YOUR -->  [:SUDAH DI MODIVIKASI:]                                                                    
                                                               -> github.com/Bjorka-id
                                                              --> OVER POWER
                                                              --> \033[32mBlack«»Naga_Team
...
def login_lagi():
	...
	panda = input(x+'['+p+'•'+x+'] masukan token (EEAB)  : ')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [Bjorka-id] %s/%s 😎 [OK] %s (^_^) [CP] %s 😂 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
